...
ServerStatus.prototype._pickupHostName = (data) => {
  var hostLists = [];
  var ec2HostTag = this.serverType === "lobby" ? config.get('ec2HostTagValueForLobby') : config.get('ec2HostTagValueForGame');
  data.Reservations.forEach((reservation) => {
    instance = reservation.Instances[0];
    instance.Tags.forEach((tag) => {
      if (tag === undefined) return;
      if (tag.Key.match(config.get('ec2HostTagKey')) && tag.Value.match(ec2HostTag)) {
