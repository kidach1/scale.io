...
Autoscale.prototype._checkBySpecifiedTime = (connNumsOfGreenHosts) => {
  // 全国対戦が1h以内に開始する時間帯かつspecifiedTimeGreenHostsNum(40台)より小さい状態の場合、specifiedTimeGreenHostsNum台までスケールアウト
  if (connNumsOfGreenHosts.length >= config.get('specifiedTimeGreenHostsNum')) return false;
  if (!this._isScalableTime(config.get('forceScaleOutStartAt'), config.get('forceScaleOutEndAt'))) return false;
...
Autoscale.prototype._checkByConnNumAverage = (connNumsOfGreenHosts) => {
  ...
  if (this._isScalableTime(config.get('scaleInAvailableStartAt'), config.get('scaleInAvailableEndAt'))
...
Autoscale.prototype._isScalableTime = (startAt, endAt) => {
  ...
  log.autoscale.info('_isScalableTime, startAt, endAt)', isScaleInAvailable, startAt, endAt);
