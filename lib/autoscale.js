...
var child_process = require('../lib/child_process');
...
Autoscale.prototype._checkByConnNumAverage = (connNumsOfGreenHosts) => {
  ...
  if (this._isScaleInAvailable() && connNumAverage <= config.get('scaleInThreshold')) this._scaleIn();
};
...
Autoscale.prototype._notifyRoomNum = (connNumsOfGreenHosts) => {
  ...
  slack.notify('マルチバトル開催数 同時' + roomNum + '部屋');
};
...
Autoscale.prototype._isScaleInAvailable = () => {
  var startAt = this._now().clone().startOf('day').add(config.get('scaleInAvailableStartAt'), 'hours');
  var endAt = this._now().clone().startOf('day').add(config.get('scaleInAvailableEndAt'), 'hours');
  var isScaleInAvailable = (this._now() >= startAt && this._now() < endAt);
  log.autoscale.info('isScaleInAvailable', isScaleInAvailable);
  return isScaleInAvailable;
};
