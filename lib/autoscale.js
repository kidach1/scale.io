...
Autoscale.prototype.checkAndApply = () => {
  ...
  this._setGreenHostsNumStream(published);
...
Autoscale.prototype._setGreenHostsNumStream = (parentStream) => {
  var _this = this;
  parentStream.subscribe((connNumsOfGreenHost) => {
    _this.greenHostsNum = connNumsOfGreenHost.length * config.get('serverTypeNum');
  });
}
  ...
  parentStream.subscribe((connNumsOfGreenHost) => {
    return Rx.Observable.fromArray(connNumsOfGreenHost)
      ...
      .filter((countOfGreenHosts) => {
        return connNumsOfGreenHost.length !== 0
          && countOfGreenHosts <= config.get('minimumGreenHostsNum');
      })
      .subscribe(() => {
        _this._scaleOut();
      });
